...
# run function
df = filter_objects_start(df, hemisphere, max_mag, size_range, object_types, s_after)
...
save_input = input("Save as csv? (Y/N): ").upper()
if save_input == "Y":
    name_choice_yn = input(f"Set custom name instead of DSO_filtered_{current_date}_{current_time[:8]}.csv (Y/N)?: ").upper()
    if name_choice_yn == "Y":
        name_choice = input("Input file name: ")
        print(f"Saving results as {name_choice}.csv") # cut off decimal from time
        df.to_csv(f'data/data_saves/{name_choice}.csv')
    else:
        print(f"Saving results as DSO_filtered_{current_date}_{current_time[:8]}.csv") # cut off decimal from time
        df.to_csv(f'data/data_saves/DSO_filtered_{current_date}_{current_time[:8]}.csv')
else:
    pass

# specify altitude and azimuth range
spec_range = input("\nSpecify Alt, Az range (Y/N)?: ").upper()
if spec_range == "Y":
    min_alt = float(input("Min Altitude: "))
    max_alt = float(input("Max Altitude: "))
    min_az = float(input("Min Azimuth: "))
    max_az = float(input("Max Azimuth: "))
    sorted_results.Alt.astype(float)
    sorted_results.Az.astype(float)
    showing = sorted_results[(sorted_results.Alt >= min_alt) & (sorted_results.Alt <= max_alt) & (sorted_results.Az >= min_az) & (sorted_results.Az <= max_az)]
    num_to_display = int(input(f"Number of objects to display out of {len(showing)}: "))
    print("Displaying select columns for the ranked objects weighted 60% for Magnitude and 40% for Size")
    print(f"Showing {num_to_display}/{len(showing)} rows\n")
    showing = showing[["ObjectNum", "Name", "Type", "Constellation", "Magnitude", "Size", "Set", "Hours_Until_Set"]].head(num_to_display)
    print(tabulate(showing, headers=showing.columns, tablefmt='grid', showindex='never'))
else:
    pass